    ...
    <svg className="chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Tafel candidate chart">
      ...
      {picks.slice().reverse().map((pick, index) => (
        <rect
          ...
          height={innerHeight}
          fill={colors[pick.rank - 1] ?? '#94a3b8'}
      ...
      {picks.map((pick) => {
        ...
        return (
          <line
            ...
            x2={xScale(x2)}
            y2={yScale(y2)}
            stroke={colors[pick.rank - 1] ?? '#334155'}
            strokeWidth={4}
            strokeLinecap="round"
          />
        );
      ...
      <g transform={`translate(${margin.left + 12}, ${margin.top + 12})`}>
        <rect width={320} height={Math.max(34, picks.length * 24 + 14)} rx={12} fill="white" opacity={0.92} stroke="#dbe3ef" />
        {picks.map((pick, index) => (
          <g key={`legend-${pick.rank}`} transform={`translate(12, ${22 + index * 24})`}>
            <line x1={0} y1={-4} x2={24} y2={-4} stroke={colors[pick.rank - 1]} strokeWidth={4} strokeLinecap="round" />
            <text x={34} y={0} fontSize={13} fill="#142033">
              pick {pick.rank} ({pick.label}): {pick.tafelSlopeMvDec.toFixed(1)}
