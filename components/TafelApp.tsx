...
import { LocalSlopeChart } from '@/components/LocalSlopeChart';
  ...
  return (
    <main className="container">
      <section className="hero">
        ...
        <p>
          Upload eta-logi or potential-current CSV data. The app runs entirely in your browser, finds a consensus Tafel region,
          expands it into three picks, estimates bootstrap confidence intervals, and exports CSV results.
        </p>
      </section>
          ...
          {result && result.picks.length > 0 && (
            <>
              <div className="panel chart-card">
                <h3>Consensus-expanded Tafel picks</h3>
                <TafelChart points={result.points} picks={result.picks} />
              </div>

              <div className="panel chart-card">
                <h3>Rolling local apparent Tafel slope</h3>
                <LocalSlopeChart slopes={result.localSlopes} picks={result.picks} />
              </div>
              ...
              <div className="panel">
                ...
                <div className="table-wrap">
                  <table>
                    <thead>
                      <tr>
                        <th>pick</th><th>range / V</th><th>slope</th><th>CI95</th><th>R2</th><th>plateau</th><th>artifact</th><th>confidence</th>
                      </tr>
                    </thead>
                    <tbody>
                      {result.picks.map((pick) => (
                        <tr key={pick.rank}>
                          <td>{pick.rank} ({pick.label})</td>
                          ...
                          <td>{pick.plateauScore.toFixed(3)}</td>
                          <td>{pick.artifactPenalty.toFixed(3)}</td>
